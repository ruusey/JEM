...
public class GeoLocServices {
	...
	public GeoLocServices() {
		Scanner s = null;
		try {
			s = new Scanner(new File("C:\\temp\\google_key.txt"));
			String key = s.nextLine();
			api = new GeoApiContext().setApiKey(key);
	...
	public boolean isInRadius(GeoLocation center, GeoLocation other, int radius) {
		...
		try {
			result = DistanceMatrixApi.getDistanceMatrix(api, new String[] { addressOne }, new String[] { addressTwo })
					.await();
		} catch (Exception e) {
